...
class ProductionConfig(AgenticRAGConfig):
    ...
    MAX_SEARCH_ATTEMPTS = 3  # Mais tentativas para melhor qualidade
    QUALITY_THRESHOLD = 0.6  # Qualidade mais alta
    DEFAULT_MAX_TOKENS = 1000  # Respostas completas
    DEFAULT_TEMPERATURE = 0.7  # Balanceado
    SEARCH_TIMEOUT_MS = 30000  # 30s timeout
    GENERATION_TIMEOUT_MS = 60000  # 60s timeout
    ENABLE_DETAILED_LOGGING = True  # Logs completos em produção
    CACHE_SEARCH_RESULTS = True  # Cache habilitado


# AgentPrompts class removed - now using DynamicPromptV2
# The prompt system has been simplified to use only system_prompt from JSON files
# See rag/dynamic_prompt_v2.py for the new implementation
